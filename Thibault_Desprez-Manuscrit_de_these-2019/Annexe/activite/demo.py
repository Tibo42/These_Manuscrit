...
class BaseDemo(Primitive):
    ...
    #je définit la fonction setup()
        ...
        #temps en fonction de la distance maximal actuel ; soit une distance max (thèorique) de 180 degrés exécuter en un temps de 1,8 seconde ; connaissance ma position_défaut la distance max est 150 degrés (avec m2) donnant un temps max de 1,5 secondes pour reprendre sa positon. on à donc le temps qui varie en fonction de la distance, c'est donc la vitesse qui est fixe. V=D/T ; 150/1,5=100 ; le moteur le plus éloigner de sa postion defaut se déplace à 100 degrés/seconde ; les autre moteurs ajuste leur vitesse (ralentissent) pour arriver à leur position defaut en même temps que le moteur le plus éloigné.
